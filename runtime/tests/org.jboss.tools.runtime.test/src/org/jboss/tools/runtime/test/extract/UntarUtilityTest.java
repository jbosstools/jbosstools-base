...
public class UntarUtilityTest {
	...
	public void testUntar() throws Exception {
		...
		InputStream resourceAsStream = getClass().getResourceAsStream("simpleWithRoot.tar");
		File fileToExtract = new File(extractFolder, "simpleWithRoot.tar");
		FileUtils.copy(resourceAsStream, new FileOutputStream(fileToExtract));
		
		File destination = new File(extractFolder, "destFolder");
		new ExtractUtility(fileToExtract).extract(destination, createOverwrite(), new NullProgressMonitor());
