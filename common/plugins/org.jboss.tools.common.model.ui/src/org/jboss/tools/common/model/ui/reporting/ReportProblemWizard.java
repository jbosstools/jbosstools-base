...
class ReportProblemWizardView extends AbstractQueryWizardView {
	...
	XModelObject reportOptions = PreferenceModelUtilities.getPreferenceModel().getByPath(ReportPreference.OPTIONS_REPORT_PROBLEM_PATH);
...
//		{ReportPreference.ATT_ATTACH_ECLIPSE_LOG, "no"}	
	...
	XEntityData sendData2 = XEntityDataImpl.create(new String[][]{
		{"SharableReportProblem"}, 
//		{ReportPreference.ATT_ATTACH_ECLIPSE_LOG, "no"}	
	});
	XEntityData infoData = XEntityDataImpl.create(new String[][]{
		{"SharableReportProblem"}, {ReportPreference.ATT_E_MAIL, "no"}, {ReportPreference.ATT_OTHER, "no"}	
	});	
	...
	private void createStackTracesControl(Composite parent) {
		...
		exceptionsObject.setAttributeValue("exceptions", formatContent(ProblemReportingHelper.buffer.getContent()));
	...
	private void createProblemControl(Composite parent) {
		...
		problemDescription = new Text(g, SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL | SWT.BORDER);
	...
	private void createStackTracesLabelControl(Composite parent) {
		...
		TextAndReferenceComponent t2 = new TextAndReferenceComponent(panel, SWT.NONE);
		d = new GridData(GridData.FILL_HORIZONTAL);
		d.horizontalIndent = 5;
		t2.setLayoutData(d);
		String text = "You have " + stackTracesCount + " stack traces to send (<a>see details</a>).";
		t2.setText(text, 300);
		t2.addReferenceListener(new ReferenceListener(){
			public void referenceSelected(String reference) {
				XActionInvoker.invoke("EditActions.Edit", exceptionsObject, null);
			}
		});
	}
	...
	private void submitProblems() {
		if(sendSupport != null) sendSupport.store();
		if(infoSupport != null) infoSupport.store();
		...
		boolean addEclipseLog = "yes".equals(ReportPreference.ATTACH_ECLIPSE_LOG_OPTION.getValue());
		if(addEclipseLog) {
...
//			ProblemReportingHelper.buffer.clean();
	...
	boolean isMessageEmpty() {
		if(sendSupport != null) {
			IModelPropertyEditorAdapter a = sendSupport.getPropertyEditorAdapterByName(ReportPreference.ATT_ATTACH_REDHAT_LOG);
			if(a != null && "yes".equals(a.getValue())) return false;
			a = sendSupport.getPropertyEditorAdapterByName(ReportPreference.ATT_ATTACH_ECLIPSE_LOG);
			if(a != null && "yes".equals(a.getValue())) return false;
		}
