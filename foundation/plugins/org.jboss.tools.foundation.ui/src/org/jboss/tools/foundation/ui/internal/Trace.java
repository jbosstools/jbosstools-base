...
public class Trace implements DebugOptionsListener {
	...
	public void optionsChanged(DebugOptions options) {
		Trace.CONFIG = options.getBooleanOption(FoundationUIPlugin.PLUGIN_ID + Trace.STRING_CONFIG, false);
		Trace.INFO = options.getBooleanOption(FoundationUIPlugin.PLUGIN_ID + Trace.STRING_INFO, false);
		Trace.WARNING = options.getBooleanOption(FoundationUIPlugin.PLUGIN_ID + Trace.STRING_WARNING, false);
		Trace.SEVERE = options.getBooleanOption(FoundationUIPlugin.PLUGIN_ID + Trace.STRING_SEVERE, false);
		Trace.FINER = options.getBooleanOption(FoundationUIPlugin.PLUGIN_ID + Trace.STRING_FINER, false);
		Trace.FINEST = options.getBooleanOption(FoundationUIPlugin.PLUGIN_ID + Trace.STRING_FINEST, false);
		Trace.RESOURCES = options.getBooleanOption(FoundationUIPlugin.PLUGIN_ID + Trace.STRING_RESOURCES, false);
		Trace.EXTENSION_POINT = options.getBooleanOption(FoundationUIPlugin.PLUGIN_ID + Trace.STRING_EXTENSION_POINT, false);
		Trace.LISTENERS = options.getBooleanOption(FoundationUIPlugin.PLUGIN_ID + Trace.STRING_LISTENERS, false);
	}
	...
	public static void trace(final String level, String s, Throwable t) {
		...
		if (Trace.STRING_SEVERE.equals(level)) {
			FoundationUIPlugin.pluginLog().logError(s, t);
		}
		if (FoundationUIPlugin.getDefault().isDebugging()) {
			final StringBuilder sb = new StringBuilder(FoundationUIPlugin.PLUGIN_ID);
			sb.append(" "); //$NON-NLS-1$
			sb.append(level);
			sb.append(" "); //$NON-NLS-1$
