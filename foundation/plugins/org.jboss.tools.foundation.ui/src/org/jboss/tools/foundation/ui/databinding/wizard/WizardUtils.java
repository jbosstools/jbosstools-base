...
import org.jboss.tools.foundation.core.jobs.JobResultFuture;
...
public class WizardUtils {
	...
	private static IStatus getStatus(final Job job, final JobResultFuture future) {
		...
		if (future.isCancelled()) {
			...
			IStatus ret = StatusFactory.errorStatus(FoundationUIPlugin.PLUGIN_ID, message);
			FoundationUIPlugin.getDefault().getLog().log(ret);
			return ret;
		}
		if (future.isDone()) {
